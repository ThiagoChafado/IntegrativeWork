...
router.get("/sellers/:shopname", async (req, res) => {
  try {
    ...
    const sellers = await db.any("SELECT s.* FROM seller s NATURAL JOIN sellerboard sl WHERE sl.shopname = $1;",[aux]);
...
router.put("/sellersedit",async (req,res) =>{
  try{
    ...
    const editing = await db.none("UPDATE seller SET sellername = $1,dtbirth = $2,pccommision = $3 WHERE sellercpf = $4 AND sellercpf in (SELECT sellercpf FROM sellerboard WHERE shopname = $5);",
